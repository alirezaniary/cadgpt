...
Read-only is permanent for pre-flight, not a stage it grows out of. When the write direction arrives, repair is a separate capability under its own rules (5.11), and even that does not retire the risk this section concentrates: an export carrying no space model at all is answered by the designer placing rooms in their own tool, and how often that happens and what it costs them is a measurement (gate 3), not something a design decision closes.
...
The observations are also the queryable artifact, and there is no second one. What the checker reads, what a report cites and what the agent queries (5.9) is this set — the enriched model itself plus the relations between subjects, not a database standing beside it. 5.3's rule that there is no separate internal model is what makes that possible.
...
**Open, and deliberately not left to whoever writes the derivation layer first.** A role is a Derived observation that is never stored, and the only evaluator named here is ifctester over IDS, which cannot compute one — so nothing above states where a pack's own selector actually runs. Materialising the role as a property contradicts 5.3 and collides in a multi-pack run: the engine is called once per pack against one model, so the second pack's assignment overwrites the first's, and 5.5's build guard forbids the property name that would tell them apart. Two candidate answers survive — a per-pack-version property namespace with each pack's run isolated, or role rules evaluated in the upstream non-IDS format 5.5 has under evaluation — and the choice is taken at the start of Phase 4 rather than here. It is recorded as a question precisely because it must not be answered locally at implementation time: the cheap local answer is the one that compiles a jurisdiction into the engine, which is I4's failure mode arriving through the door 5.3 was written to close.
...
**Also rejected: a graph database as a spatial index.** The property graph a spatial question needs already exists — topologicpy's dual graph supplies containment and connectivity, and `Related` observations carry the edges with provenance attached. Standing a Neo4j beside the model adds a second store to keep in sync with the first and answers nothing the first cannot. The question recurs often enough to be recorded rather than re-argued.
...
**Nothing that is not an established PASS is counted as a PASS.** The values stay three while this section keeps naming distinct outcomes — a near-miss inside tolerance, a deemed-to-satisfy route not followed, an applicability that could not be determined — so each needs a stated home rather than an implementer's guess, and the home is the same for all of them: INDETERMINATE, carrying its own reason code, and additionally reported as its own count beside the three. A near-miss is INDETERMINATE even where the measurement clears the limit, because "within the declared tolerance of the limit" is not a verdict the measurement established; the margin states what was measured and the reason states why it is not a pass. This crowds INDETERMINATE and makes a first run read worse than a two-valued tool would, which is the intended direction — the count that is allowed to be uncomfortable is the one nobody is buying.
...
The denominator has the same defect one level up, and it is closed here rather than discovered later: "clauses in force" comes from the pack's own clause index, which the pipeline of 8 produces alongside the rules, so a chapter nobody ingested is absent from both sides of the fraction and a partial pack reads as fully scoped. Until a clause index is enumerated independently of rule authoring, coverage is reported bounded-below and states its own limit — "at least 12 of the 80 indexed provisions; chapters 4 to 7 not indexed" — never a bare fraction implying the denominator is the whole code. The independent enumeration step is the real fix and waits for the first authored pack, where its cost against gate 1's throughput is measurable instead of guessed.
...
**What is checked is the model; what is submitted is sheets.** The artifact a plan reviewer marks up is the drawing set, and an office that models the geometry but drafts its documentation in 2D over it can submit sheets that diverge from the model this system checked — detailing drawn on a view, a hand-typed schedule, an area table in a titleblock. That divergence is out of scope and unchecked, and I7 requires saying so plainly rather than allowing "the model complies" to be read as "the submission complies"; every report names the model it checked. The mitigation already exists and costs nothing: the marked sheets of 5.8 are generated from the checked model, so an office that submits those has no gap to disclose. How large the gap is in this market is what gate 2 measures.
...
**The tool surface is closed and typed.** Every tool is a declared schema; a call that does not validate is rejected and returned to the model as an error, never coerced into something that will run. There is no general-purpose escape hatch — no evaluate-this-script tool, no pass-through to the host's macro engine, no free-text query compiled at runtime. This is I2 enforced at the transport rather than at the module boundary: a model that cannot author geometry but can hand a script to the host authors geometry anyway, and several existing CAD MCP servers ship exactly that tool. It is also the answer to the failure mode that dominates code-generating agents pointed at CAD APIs, which is confident invocation of methods that do not exist. A closed enumerated surface cannot be hallucinated into; it can only be called wrongly, and a wrong call fails validation with a message the model can act on. The constraint is on what we expose, not on what the host can do — pyRevit is a Python script host and we chose it deliberately (5.10) — and it holds on every host regardless of what that host is capable of. Nor is a rejected call ever repaired into a valid one: coercing a string to a number, clamping an argument into range or defaulting a missing one is invention wearing a type.

**Model queries return structure, not prose.** The agent reads the observation store and the topology relations of 5.4 — subjects, properties, conventions, values, and the related-to edges between them — never a flattened text serialization of the model. Serializing a building into paragraphs and retrieving over them discards exactly the containment and connectivity that every spatial question turns on, and it discards them silently, because the answers stay fluent. The observation tuple is already the right retrieval unit and it arrives with its provenance and confidence attached; there is no second index to build.

**The agent is measured against the same fixtures the packs are.** Its quality is a number the oracle produces, not an impression a demo produces: run it over the known-good and known-bad models of 5.5 and count what it got right. Section 2's claim only holds if the oracle grades the agent too.
...
**The data boundary is stated, not assumed.** No client model, drawing, or derived observation leaves the deployment. The one outbound inference call anywhere in the system is codification (8), which reads published regulatory text and never sees a customer model — which is why that service may use a hosted API without moving the boundary. A deployment may point the assistance layer at an external or customer-supplied endpoint instead of the local one, and that exception is bound rather than merely discouraged: it is deployment configuration and never a per-request choice, it is never a default, and it is enforced by which service is permitted to hold a model handle rather than by anyone remembering the rule. An exception governed by policy is the exception that eats the invariant. Offices working under client NDAs cannot adopt a tool whose data path they have to take on trust, and a default is only meaningful if its exceptions are enumerated.

The contract therefore has a named subject on each side, because "no inference in the system" is not what is meant and cannot be enforced: the codification service (8) and this assistance layer may import an inference client; the checking engine, the derivation layer and report generation may not, and no component that may hold a model handle may reach an external endpoint.
...
**The host is single-threaded, and that decides the connector's shape.** Desktop CAD applications run their API on the UI thread and crash or deadlock when it is called from anywhere else, while the connector's inbound channel is asynchronous by construction, because requests arrive from the server whenever the agent makes them. The connector contract is therefore that no host API call is ever made from a background thread, and each host names the one idiom it is marshalled through: an `ExternalEvent` or `Idling` handler on Revit; on Archicad nothing, because the Python Automation API is already an out-of-process endpoint and the marshalling is the vendor's problem; a modal or timer handler on Blender. The connector holds a queue, not a thread pool. Long reads are chunked and cancellable so that a model scan never presents as a frozen application. That requirement lands on the v0 pre-flight tool on its first day, not later: a read-only inspector that locks the host for two minutes is uninstalled before it finds anything. This is stated here rather than left to implementation because it is not retrofittable — it is the difference between a connector that is a queue and one that is a thread pool, and the second one does not work. It also reinforces a choice already made in 5.1: the client is an executor and a renderer with no logic in it, so the host-side surface is short by design and what has to stay cancellable is the enumeration of a large model, not a computation.
...
**A write is a transaction, and the loop closes on a check.** Every write enters the model as one named transaction, so the rollback mechanism is the application's own undo stack with our move named in it — the escape hatch the user already trusts rather than a parallel one we invent. The move is proposed, written, and immediately re-checked, and the findings delta is reported for the user to decide on. It is not auto-reverted: a legitimate move can resolve a serious finding and raise a lesser one — a corridor widened to the required clear width takes a room below its minimum area — and a tool that silently undoes that is fighting the designer, and has quietly given the evaluator a veto over what may exist, which inverts both I1 and the non-goal in 10. The delta is also not a simple count under three-valued logic, because a write can make previously inapplicable checks apply and come back INDETERMINATE for want of an observation. Automatic revert is reserved for the hard class: the write failed, the host transaction errored, or the result cannot be measured because post-write derivation will not run. This is section 2 arriving — the compile-test-fix loop a coding agent gets for free, assembled by hand — and it exists only because the v0 engine is there to be the test. Parameters are validated at the generator boundary before anything is written, so an out-of-domain value is a rejected call rather than geometry somebody has to notice.

**Repair is authoring, and the same rule decides who did it.** The gap 5.2 concentrates — an export with rooms missing or unbounded — is closed at source by the designer, and once the write direction exists the agent can help without closing it for them. It may select the host's own room command and navigate to the place; it may never supply that command's geometric arguments. No boundary polygon, no extent, no count: the moment we compute the region and pre-fill it, we authored the space and the host was our pen, which is exactly the click-shaped path around I1 that 5.7 names. The bulk case is the host's own automatic room-placement command, invoked by the user — the algorithm deciding where the rooms are is then the vendor's, one the designer already owns and already uses. The test is what survives us: they close the tool, never run it again, and the spaces are still in their file, indistinguishable from spaces they drew last Tuesday. That is also why a repaired space carries no special provenance tag — provenance marks a fact we could not verify (5.7), and this one we measure like any other.

Past a handful of missing rooms this stops being repair and becomes a rebuild, and the honest output is one instruction rather than two hundred confirmations: this model has no space model, place rooms in your tool and export again. Two hundred rubber-stamped dialogs is invention with a signature page, and no amount of consent in the interface changes who computed the boundaries.
...
It anticipates upstream moving, not upstream stopping, which is a different risk and is named here rather than answered. Three entries are sole sources for checks v0 ships: `topologicpy` for space connectivity, travel distance, dead-end length and fire-separation adjacency — the egress and fire-separation checks of 9, and the property graph both 5.4 and 5.9 rest on, which is also why 5.4's rejection of a graph database depends on it; `ifcgref` for the georeferencing join the entire parcel channel needs, and with it site coverage, density and setback; `IFC_BuildingEnvExtractor` for footprint and envelope. If one stalls, those checks go dark, and I3 rules out rebuilding them. Whether vendoring a fork is a permitted exception to I3 is open.
...
- **Cross-document review of the submission set** — specification books, product submittals, and callout alignment between sheets. The cost of that misalignment is real, but it lands in a different project phase on a different buyer than permit-stage design, and the parsing it would need puts a language model back in the evidence path: a callout read off a PDF is an extracted claim, not a measurement (I1, 5.2). We check the model and disclose that we checked the model (5.7). *Reopens if a jurisdiction mandates a model deliverable — which dissolves the problem rather than requiring a parser — if the product moves to the contractor and construction-administration buyer, or if gate 2 returns a hybrid-dominant market.*
...
**Gate 2 — market shape.** In the first target market, how do offices that actually submit to plan review author their work? This decides whether v0's import path has enough reachable users to be worth shipping, and whether v2's connector has a market. The question has three answers, not two, and the third is the easy one to miss: an office may model the geometry and still draft the submitted sheets in 2D over it. Ask twenty offices which of the three they are; no files and no engineering required.

- If a workable fraction models and submits from the model: build as specified.
- If hybrid dominates — modelled geometry, drafted submission — v0 still checks the real building, but its findings do not transfer to the sheets the reviewer marks up (5.7). Lead with the marked sheets of 5.8 as the submission artifact, which closes the gap by making the checked model the source of the drawing set rather than a document beside it.
...
| Pre-flight is read-only permanently; repair arrives with the write direction and may select the host's own command but never supply its geometric arguments; past a handful of rooms the answer is one instruction, not two hundred confirmations | 5.2, 5.11 | Never. Consent in the interface does not change who computed the boundary |
...
| No separate graph database; topologicpy's dual graph plus `Related` observations is the property graph, and the observation set is the only queryable artifact | 5.4 | — |
...
| The instrument behind a parcel entitlement is stored and cited as a document; nothing extracts its parameter | 5.5 | — |
...
| Findings and applicability are three-valued; nothing that is not an established PASS is counted as a PASS; coverage reported per run before findings, with the effective rule-set size, and bounded-below until clause indexes are enumerated independently of rule authoring | 5.7, I7 | Never for the values. The reason taxonomy grows; the coverage denominator firms up with the first authored pack |
| Every rule declares a compliance route; an unmet deemed-to-satisfy rule is reported as not following that route | 5.7 | — |
| Every comparison declares a tolerance policy and every finding reports margin; rounding is separate; routing thresholds get zero tolerance; a near-miss and a route not followed are INDETERMINATE with their own reason and their own count, never PASS | 5.7 | — |
...
| We check the model, not the submitted sheet set, and every report says so; cross-document review of specs, submittals and sheet callouts is a non-goal | 5.7, 10 | A mandated model deliverable, a move to the construction-administration buyer, or a hybrid-dominant answer to gate 2 |
...
| The agent's tool surface is closed and typed; no evaluate-script tool and no pass-through to the host's macro engine | 5.9 | Never — this is I2 at the transport |
| Agent retrieval runs over the observation store and topology relations, never a text serialization of the model | 5.9 | — |
| No client model, drawing or observation leaves the deployment; codification over published regulatory text is the only outbound inference call; an external assistance endpoint is deployment configuration, never a default and never per-request | 5.9, 8 | — |
| The connector is offline and local, deferred past v0, MCP over localhost, not a platform the user logs into | 5.10 | — |
| The connector marshals every host call onto the application's UI thread and is a queue, not a thread pool; long reads are chunked and cancellable | 5.10 | Never — the constraint belongs to the host, not to us |
| A write is one named host transaction re-checked immediately; the findings delta is reported for the user to decide, and automatic revert is reserved for a failed or unmeasurable write | 5.11 | — |
...
| Rules ship as a selectable catalogue (jurisdiction, region, version); user upload is post-MVP — its API remains in the backend, unsurfaced in the UI, retired only as its own task; the catalogue is a separate model from the tenant-owned rule set so `for_tenant` stays total | 5.5, plan Phase 3 | A jurisdiction that will not express as IDS files plus metadata — fixed in the format, never a private dialect |
| The first iteration reports and does not act; overlay, marked sheets and BCF leave the MVP, and acting on findings arrives with the agent layer's permission levels (auto, edit, ask-first) | 5.8, 5.9, 9 | A first user who cannot locate the element a finding names — then a viewer is a legibility fix, not a workflow feature |
| The MVP deliverable is a generated Markdown report file whose URL sits on the job record; the in-app view stays beside it, not under it | 5.7, 5.8, 9 | The file becoming the only artifact anyone opens |
| The upload ceiling is derived from measured worker memory, not chosen as a round number; async removes the time constraint, not the memory one, and an oversized job fails with a named reason rather than being redelivered | 5.2, 5.7 | Out-of-core extraction landing upstream in ifcopenshell |
